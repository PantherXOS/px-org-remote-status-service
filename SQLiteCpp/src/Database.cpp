...
void Database::loadExtension(const char* apExtensionName, const char *apEntryPointName)
{
    ...
    // Security warning:
    // It is recommended that the SQLITE_DBCONFIG_ENABLE_LOAD_EXTENSION method be used to enable only this interfaces.
    // The use of the sqlite3_enable_load_extension() interfaces should be avoided to keep the SQL load_extension()
